...
void *tree_sitter_biscuit_language_external_scanner_create() {
...
void tree_sitter_biscuit_language_external_scanner_destroy(void *p) {
...
unsigned tree_sitter_biscuit_language_external_scanner_serialize(void *p, char *buffer) {
    unsigned i = 0;
    Scanner *scanner = (Scanner*) p;
...
void tree_sitter_biscuit_language_external_scanner_deserialize(void *p, const char *buffer, unsigned n) {
    Scanner *scanner = (Scanner*) p;
    if (n > 0){
        unsigned i = 0;
        scanner->initialized = true;
...
bool tree_sitter_biscuit_language_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    if (lexer->lookahead == 0) {
        // tsv_advance(lexer);
        return false;
    }
